...
struct ringbuffer *ringbuffer_create(unsigned int size)
{
	...
	ring_buf = (struct ringbuffer *)malloc(sizeof(*ring_buf));
	if (!ring_buf) {
		perror("malloc()");
		return NULL;
	}
...
unsigned int ringbuffer_to_fd(int file_fd, struct ringbuffer *ring_buf, unsigned int len)
{
	unsigned int l;
	...
	write_num = write(file_fd, ring_buf->data + (ring_buf->read_pos & (ring_buf->size - 1)), l); 
	if (write_num < 0){ 
		perror("write()");
		exit(1);
	}
	...
	if (len - l) {
		write_num = write(file_fd, ring_buf->data, len - l); 
		if (write_num < 0){ 
			perror("write()");
			exit(1);
		}
